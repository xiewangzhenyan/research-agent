...
from app.services.memory import content_hash, memory_state
...
async def execute(job_id, check_connection):
    async with get_worker_db_context() as db:
        ...
        if source is None or source.role != "user" or not extraction.eligible(source.content):
            finish(job, "skipped")
            return
        if len(await repo.proposals()) >= 20:
            finish(job, "skipped", "待确认建议已达 20 条，请先整理")
            return
        items = [i for i in await repo.list() if memory_state(i) == "active"]
        related = {r["id"] for r in select_memories(source.content, items)["items"]}
        targets = [i for i in items if str(i.id) in related][:8]
    ...
    try:
        result, usage = await extraction.infer(text, history, targets, config)
        ...
        async with get_worker_db_context() as db:
            ...
            if source is None or source.role != "user" or source.content != text:
                finish(job, "cancelled")
                return
            pending = await repo.proposals()
            hashes = {p.content_hash for p in pending}
            count = 0
            for candidate, target in extraction.validated_candidates(result, text, targets):
                if len(pending) + count >= 20:
                    break
                digest = content_hash(candidate.content)
                if digest in hashes or await repo.duplicate(digest):
                    continue
                if target:
                ...
                hashes.add(digest)
                count += 1
            job.result_count, job.usage = count, usage
