...
class ConversationService:
        ...
        *,
        ...
        if {
            ...
            if user_id is None or conversation.user_id != user_id:
                raise NotFoundError(message="仅会话所有者可以修改知识范围")
            from app.services.knowledge import KnowledgeService

            base_ids = [
                UUID(str(v))
                for v in update_data.get(
                    "active_knowledge_base_ids", conversation.active_knowledge_base_ids
                )
            ]
            document_ids = update_data.get(
                "active_knowledge_document_ids", conversation.active_knowledge_document_ids
            )
            await KnowledgeService(self.db, user_id).validate_scope(
                base_ids, [UUID(str(v)) for v in document_ids] if document_ids is not None else None
            )
            if "active_knowledge_document_ids" in update_data and document_ids is not None:
                update_data["active_knowledge_document_ids"] = [str(v) for v in document_ids]
        if (
    ...
    async def add_message(
        ...
        *,
        ...
        return await conversation_repo.create_message(
            ...
            effective_config=data.effective_config.model_dump() if data.effective_config else None,
        )
    ...
    async def link_files_to_message(
        ...
        await self.list_attached_files(file_ids, user_id=user_id)
    ...
    async def list_attached_files(self, file_ids: list[str], *, user_id: UUID) -> list[Any]:
        ...
        files = await chat_file_repo.get_many(self.db, ids, user_id=user_id)
        if len(files) != len(ids):
            raise NotFoundError(message="附件不存在或无权访问")
        return files
