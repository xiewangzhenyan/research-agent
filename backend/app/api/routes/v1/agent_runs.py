...
async def create(
    ...
    return await AgentRunService(db, user.id, project_id=project_id).create(data)
...
async def list_runs(
    ...
    return await AgentRunService(db, user.id, project_id=project_id).repo.list(
        user.id, offset, project_id=project_id
    )
...
async def get(
    ...
    return await AgentRunService(db, user.id, project_id=project_id).get(run_id)
...
async def events(
    ...
    service = AgentRunService(db, user.id, project_id=project_id)
...
async def cancel(
    ...
    return await AgentRunService(db, user.id, project_id=project_id).cancel(run_id)
...
async def resume(
    ...
    return await AgentRunService(db, user.id, project_id=project_id).resume(run_id, data)
...
async def artifacts(
    ...
    return await RunArtifactService(db, user.id, project_id=project_id).list(run_id)
...
async def download_artifact(
    ...
    item = await RunArtifactService(db, user.id, project_id=project_id).get(run_id, artifact_id)
    return Response(
        item.content,
...
async def delete_artifact(
    ...
    await RunArtifactService(db, user.id, project_id=project_id).delete(run_id, artifact_id)
