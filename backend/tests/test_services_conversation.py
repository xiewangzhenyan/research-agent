...
class TestConversationServiceLinkFiles:
    ...
    async def test_link_files_calls_db(self, service: ConversationService):
        ...
        with (
            patch(
                "app.services.conversation.chat_file_repo.get_many", new_callable=AsyncMock
            ) as get_many,
            patch(
                "app.services.conversation.chat_file_repo.link_to_message", new_callable=AsyncMock
            ) as link,
        ):
            get_many.return_value = [MagicMock(), MagicMock()]
            await service.link_files_to_message(msg_id, file_ids, user_id=owner_id)
            get_many.assert_awaited_once_with(
                service.db, {UUID(fid) for fid in file_ids}, user_id=owner_id
            )
