...
it("rejects a missing login cookie without making an upstream request", async () => {
  const response = await GET(new NextRequest("https://app.test/api/agent/capabilities"));
...
it("translates the login cookie into backend authorization and prevents shared caching", async () => {
  ...
  const response = await GET(new NextRequest("https://app.test/api/agent/capabilities", { headers: { cookie: "access_token=test-session" } }));
  expect(backendFetch).toHaveBeenCalledWith("/api/v1/agent/capabilities", { headers: { Authorization: "Bearer test-session" }, cache: "no-store" });
...
it("preserves authentication failure without exposing upstream errors", async () => {
  ...
  const response = await GET(new NextRequest("https://app.test/api/agent/capabilities", { headers: { cookie: "access_token=expired" } }));
  expect(response.status).toBe(401);
