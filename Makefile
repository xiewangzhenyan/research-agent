...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app agent db upgrade
	@echo ""
	@echo "🚀 Dev stack ready:"
	@echo "   API:      http://localhost:38001"
	@echo "   Docs:     http://localhost:38001/docs"
	@echo "   Admin:    http://localhost:38001/admin"
	@echo "   Frontend: http://localhost:38000  (run 'make dev-frontend' or 'cd frontend && bun dev')"
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		...
		&& echo "  ✅ Admin created. Login at http://localhost:38001/admin"; \
	fi
...
dev-frontend:
	...
	@echo ""
	@echo "✅ Frontend at http://localhost:38000  (backend must be up — 'make dev')"

# === Staging: built images, no bind mounts (production-like, local DB) ===
stage:
	docker compose -f docker-compose.yml up -d --build
	$(call _wait_for_db,docker-compose.yml)
	docker compose -f docker-compose.yml exec -T app agent db upgrade
	@echo "✅ Staging stack at http://localhost:38001"
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app agent db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run agent db migrate -m "initial" || true
	cd backend && uv run agent db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend agent db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend agent db upgrade

db-downgrade:
	uv run --directory backend agent db downgrade

db-current:
	uv run --directory backend agent db current

db-history:
	uv run --directory backend agent db history

# === Server ===
run:
	uv run --directory backend agent server run --reload

run-prod:
	uv run --directory backend agent server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend agent server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend agent user create-admin

user-create:
	uv run --directory backend agent user create

user-list:
	uv run --directory backend agent user list

# === Celery ===
celery-worker:
	uv run --directory backend agent celery worker

celery-beat:
	uv run --directory backend agent celery beat

celery-flower:
	uv run --directory backend agent celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/agent"
